...
public class Utilities extends Controller {
    ...
    */
    ...
    /**
     * Many events have either a "creator" or an "owner" property depending on whether
     * they were scraped from FQL or Graph. The creator is just an id, however, the owner
     * is a json struct where the id is embedded inside.
     * This method normalizes the difference in the database by extracting the id from the
     * owner struct.
     * @param  event Event from which to extract owner/creator.
     * @return String creator/owner id
     */
    ...
    /**
     * Many events have either a "pic" or an "picture" property depending on whether
     * they were scraped from FQL or Graph. The pic is just a url, however, the picture
     * is a json struct where the url is embedded inside.
     * This method normalizes the difference in the database by extracting the url from the
     * picture struct.
     * @param  event Event from which to extract pic/picture.
     * @return String pic/picture url
     */
    ...
    */
    public static boolean filter_event(MyEvent event) {
        ...
        try {
            ...
            // TODO: check to makesure country and state are not blank before returning false
            // if (!country.equalsIgnoreCase("United States")) return false;
            // if (!state.equalsIgnoreCase("Illinois")) return false;

            // if(latitude,longitude is less than 1500 meters from center of NU) return true;
            if (city.equalsIgnoreCase("evanston")) return true;
